...
double getNewTemp(Box *box) {
    double waat = box->temp * box->uncommonEdgeLength;

    for (int i = 0; i < box->totalNeighborsCount; i++) {
        waat += box->allNeighbors[i].commonEdgeLength * boxes[box->allNeighbors[i].id].temp;
        if(box->id == 6)printf("waat: %lf\n", waat);
    }

    waat /= box->perimeter;

    return box->temp + (waat - box->temp) * AFFECT_RATE;
}
...
int main(int argc, char **argv) {
    ...
    for (i = 0; hasConverged() == 0; i++) {
        for (j = 0; j < boxCount; j++) {
	    newTemp[j] = getNewTemp(&boxes[j]);
	    printf("%d - %lf\n",j,newTemp[j]);
        }
	printf("\n");
        for (j = 0; j < boxCount; j++) {
